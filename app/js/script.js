/**
 * Created by MackD on 21.11.2016.
 */
if (window.jQuery == undefined) {
    document.write("<sc" + "ript src='http://olgayurkovskaya.e-autopay.com/js/jq.js' type='text/javascript'></scr" + "ipt>");
}
...
function EAcheckForm(form) {
    ...
    // записываем в куки покупку
	var day = getCurrentDay();
    setCookie('day_' + day, 'true', 365, false);
    return true;
}
...
function getData(action) {
    ...
    if (action == 'region') {
        ...
        if (country == '') return;
        var url = 'http://olgayurkovskaya.e-autopay.com/adminka/form_generator/get_form_list.php?country=' + country + '&form_charset=' + form_charset;
    } else if (action == 'city') {
        var region = document.getElementById('region_id').value;
        if (region == '') return;
        var url = 'http://olgayurkovskaya.e-autopay.com/adminka/form_generator/get_form_list.php?region=' + region + '&form_charset=' + form_charset;
    } else {
        return;
    }
...
function hideElement(obj) {
    ...
    switch (type) {
        case 'ea_custom_region':
            if (obj.checked) {
                jQuery('#ea_product_form_29959 #ea_normal_region select').val('');
                ...
                jQuery('#ea_product_form_29959 #ea_special_region input').val('');
...
window.onload = function () {
    ...
    if (current_domain !== 'olgayurkovskaya.e-autopay.com') {
...
//передаем ID товаров при клике
//$(document).ready(function () {
//    $('.btnp').click(function (e) {
//        var tovarid = $(this).attr('data-tovarid');
//        var formid = $(this).attr('data-formid');
//
//        $('#tovar_id').val(tovarid);
//        $('#form_id').val(formid);
//    });
//
//    // проверяем наличие куки и обрабатываем
//	var day = getCurrentDay();
//    var already = getCookie('day_' + day, false);
//
//    if (already !== 'null'){
//        $(".product").empty();
//        $(".todat").empty();
...
/*
* Возвращаем текущий день на основании url
*/
function getCurrentDay()
{
	var location = window.location.href;
	var pos = location.indexOf("_");
	var day = location.substr(pos + 1, 1);
...
jQuery(".equalh").each(function () {
    if (jQuery(this).height() > priceBlockHeight) {
        priceBlockHeight = jQuery(this).height();
    }
});
jQuery(".equalh").height(priceBlockHeight);
